    ...
    bool Map::load(const std::string& mapFile) {
        ...
        for(size_t y = 0; y < map.size(); y++) {
            for(size_t x = 0; x < map[y].size(); x++) {
                ...
                auto sprite = new SpriteAgent(data["tiles"][std::to_string(map[y][x])]["src"], vec3(x, map.size() - y - 1, 0));
        ...
        for(auto coll : data["walls"]) {
            ...
            auto c = new ColliderSpriteAgent(
                colldata["src"], vec2(colldata["coll"]["x"], colldata["coll"]["y"]), vec2(colldata["coll"]["w"], colldata["coll"]["h"]), 
